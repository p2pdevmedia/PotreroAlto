...
const GRADE_OPTIONS = ['', '<5a', '5a', '5a+', '5b', '5b+', '5c', '5c+', '6a', '6a+', '6b', '6b+', '6c', '6c+', '7a', '7a+', '7b', '7b+', '7c', '7c+', '8a', '8a+', '8b', '8b+', '8c', '8c+', '9a', '9a+', '>9a'];
...
function issuesToFieldErrors(issues = []) {
  ...
  return issues.reduce((accumulator, issue) => {
    ...
    const issueMessage = issue.message === 'Invalid input'
      ? `${fieldLabel}: valor inválido.`
      : issue.message;
                    ...
                    return (
                      <>
                        ...
                        <div className="mb-1 grid gap-2 md:grid-cols-4">
                          <select value={selectedRoute.grade ?? ''} onChange={(event) => updateRoute(selectedSubsector.id, selectedRoute.id, 'grade', event.target.value)} className="rounded border border-slate-700 bg-slate-900 px-2 py-1 text-sm text-slate-100">
                            <option value="">Sin grado</option>
                            {GRADE_OPTIONS.filter((option) => option).map((gradeOption) => <option key={gradeOption} value={gradeOption}>{gradeOption}</option>)}
                          </select>
                        ...
                        {fieldErrors.grade ? <p className="mb-2 text-xs text-red-300">{fieldErrors.grade}</p> : null}
