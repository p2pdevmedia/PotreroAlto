...
function buildCodexSessionLink(message) {
  ...
  return `https://chatgpt.com/?q=${encodeURIComponent(normalizedMessage)}`;
}
          ...
          <div className="space-y-4">
            ...
            <section className="space-y-2 rounded-xl border border-slate-700/70 bg-slate-900/50 p-3">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <button
                  type="button"
                  disabled={!pendingChangeLines.length}
                  onClick={async () => {
                    if (!pendingChangeLines.length) {
                      return;
                    }

                    if (typeof window === 'undefined' || !window.navigator?.clipboard) {
                      setError('No se pudo copiar automáticamente. Copiá la diferencia manualmente.');
                      return;
                    }

                    const diffMessage = pendingChangeLines.map((line) => `- ${line}`).join('\n');
                    await window.navigator.clipboard.writeText(diffMessage);
                    setMessage('Diferencia copiada al portapapeles.');
                  }}
                  className="rounded border border-slate-500/60 px-3 py-1 text-xs font-semibold text-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  Copiar diferencia
                </button>
              </div>
              ...
              <section className="space-y-2 rounded-xl border border-emerald-500/40 bg-emerald-900/10 p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    <a
                      href={buildCodexSessionLink(codexMessage)}
                      target="_blank"
                      rel="noreferrer"
                      className="rounded border border-sky-400/60 px-3 py-1 text-xs font-semibold text-sky-100"
                    >
                      Abrir Codex con mensaje
                    </a>
                  </div>
                </div>
                <textarea
