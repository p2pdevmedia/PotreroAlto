      ...
      {!barsOnly ? (
        <header className={`flex items-end justify-between gap-4 ${compact ? 'mb-2' : 'mb-4'}`}>
          <div>
            ...
            {!compact ? <p className="text-xs text-slate-400">{t(language, 'routesByGrade')}</p> : null}
          </div>
          <p className={`${compact ? 'text-[10px]' : 'text-xs'} text-slate-300`}>
            {totalRoutesWithGrade} {t(language, 'withGrade')}
          </p>
        </header>
      ) : null}
        ...
        {GRADE_BUCKETS.map((grade, gradeIndex) => {
          ...
          return (
            <div
              ...
              aria-label={`${count} ${t(language, 'routes')} ${t(language, 'grade')} ${grade}`}
              title={`${grade}: ${count} ${t(language, 'routes')}`}
            >
